...
switch plottype
    ...
    case 2
        for i = 1:ndays
            figure(109);
            metind = ibegm+144*(i-1):ibegm+144*i-1; % for meteo
            chemind1 = ibegc1+1440*(i-1):ibegc1+1440*i-1; % for jNO2
            chemind2 = ibegc2+1440*(i-1):ibegc2+1440*i-1; % for NO2, NO, O3
            
            subplot(2,2,1); plot(NO2(chemind2),'color',cvec(i,:)); %hold on;
            title('NO2'); xlabel('min'); ylabel('ppbv'); xlim([0 1440]); ylim([0 max(NO2(:))]);
            subplot(2,2,2); plot(NO(chemind2),'color',cvec(i,:)); %hold on;
            title('NO'); xlabel('min'); ylabel('ppbv'); xlim([0 1440]); ylim([0 max(NO(:))]);
            subplot(2,2,3); plot(jNO2(chemind1),'color',cvec(i,:)); %hold on;
            title('jNO2'); xlabel('min'); ylabel('1/s'); xlim([0 1440]); ylim([0 max(jNO2(:))]); 
            subplot(2,2,4); plot(O3(chemind2),'color',cvec(i,:)); %hold on;
